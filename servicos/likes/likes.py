...
def get_info():
    return jsonify(
        descricao = "gerenciamento de curtidas da coffeestore",
        versao = "1.0"
    )

@servico.get("/likes_por_feed/<int:id_do_feed>")
def likes_por_cafe(id_do_feed):
...
def curtir(conta, id_do_feed):
    ...
    try:
        ...
        conexao.rollback()
        resultado = jsonify(situacao = "erro", erro = "erro curtindo o café")
...
def descurtir(conta, id_do_feed):
    ...
    try:
        ...
        conexao.rollback()
        resultado = jsonify(situacao = "erro", erro = "erro descurtindo o café")
